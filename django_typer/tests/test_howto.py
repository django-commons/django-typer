...
from django_typer.tests.utils import rich_installed, run_command
from io import StringIO
import pytest


@override_settings(INSTALLED_APPS=["django_typer.tests.apps.howto"])
class TestDefaultCmdHowto(TestCase):
    cmd = "default_cmd"

    def test_howto_default_cmd(self):
        from django_typer.tests.apps.howto.management.commands.default_cmd import (
            Command,
        )
...
class TestInitializerHowto(TestCase):
    ...
    def test_howto_initializer(self):
        from django_typer.tests.apps.howto.management.commands.initializer import (
            Command,
        )
...
class TestInheritHowto(TestCase):
    ...
    def test_howto_inherit(self):
        from django_typer.tests.apps.howto.management.commands import (
            upstream,
            downstream,
        )
...
class TestPluginHowto(TestCase):
    ...
    def test_howto_plugin(self):
        from django_typer.tests.apps.howto.management.commands import upstream2
...
class TestPrintingHowto(TestCase):
    ...
    def test_howto_printing(self):
        from django_typer.tests.apps.howto.management.commands.printing import (
            Command,
        )
